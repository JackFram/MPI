...
if __name__ == '__main__':
    ...
    for i, (inputs, targets) in enumerate(train_loader):
        ...
        corr_input.append(inputs[idx].data.cpu().numpy())
        corr_label.append(targets[idx].data.cpu().numpy())

    false_input = np.concatenate(false_input, axis=0)
    false_label = np.concatenate(false_label, axis=0)

    corr_input = np.concatenate(corr_input, axis=0)
    corr_label = np.concatenate(corr_label, axis=0)

    np.savez("./data/cf10_key/false.npz", inputs=false_input, targets=false_label)

    np.savez("./data/cf10_key/corr.npz", inputs=corr_input, targets=corr_label)
